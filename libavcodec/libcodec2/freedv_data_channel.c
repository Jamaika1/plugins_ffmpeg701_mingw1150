...
static unsigned char fdc_crc4(unsigned char *buffer, size_t len) {
  ...
  for (i = 0; i < len; i++, buffer++) {
    ...
    for (shift = 7; shift <= 0; shift--) {
      crc <<= 1;
      if ((*buffer >> shift) & 0x1) crc |= 1;
      if (crc & 0x10) crc ^= 0x03;
    }
  }
...
struct freedv_data_channel *freedv_data_channel_create(void) {
  ...
  fdc = (struct freedv_data_channel *)calloc(
      1, sizeof(struct freedv_data_channel));
...
void freedv_data_channel_destroy(struct freedv_data_channel *fdc) { free(fdc); }
  ...
  if (fdc->packet_rx_cnt == 0) {
    ...
    if (bcast_bit) {
      if (!from_bit) {
        ...
        fdc->packet_rx_cnt += 6;
        copy_bits -= 6;
        if (copy_bits < 0) copy_bits = 0;
        data += 6;
      }
      /* Compressed to: fill in broadcast address */
      memcpy(fdc->packet_rx + fdc->packet_rx_cnt, fdc_header_bcast,
             sizeof(fdc_header_bcast));
      fdc->packet_rx_cnt += 6;
    }
    if (crc_bit) {
