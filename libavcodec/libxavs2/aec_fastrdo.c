 ...
 */
static
int aec_write_intra_pred_mode_fastrdo(xavs2_t *h, aec_t *p_aec, int ipmode)
{
...
/* ---------------------------------------------------------------------------
 */
int aec_write_split_flag_fastrdo(xavs2_t *h, aec_t *p_aec, int i_cu_split, int i_cu_level)
{
...
/* ---------------------------------------------------------------------------
 */
int write_sao_mergeflag_fastrdo(xavs2_t *h, aec_t *p_aec, int avail_left, int avail_up, SAOBlkParam *p_sao_param)
{
    int b_merge_left = 0;
    int b_merge_up;
    int val = 0;
...
/* ---------------------------------------------------------------------------
 */
int write_sao_mode_fastrdo(xavs2_t *h, aec_t *p_aec, SAOBlkParam *saoBlkParam)
{
...
/* ---------------------------------------------------------------------------
 */
static int aec_write_sao_offset_fastrdo(xavs2_t *h, aec_t *p_aec, int val, int offset_type)
{
    /* ---------------------------------------------------------------------------
     */
...
/* ---------------------------------------------------------------------------
 */
int write_sao_offset_fastrdo(xavs2_t *h, aec_t *p_aec, SAOBlkParam *saoBlkParam)
{
    int rate = 0;
    ...
    if (saoBlkParam->typeIdc == SAO_TYPE_BO) {
        ...
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[bandIdxBO[0]], SAO_CLASS_BO);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[bandIdxBO[1]], SAO_CLASS_BO);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[bandIdxBO[2]], SAO_CLASS_BO);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[bandIdxBO[3]], SAO_CLASS_BO);
    } else {
        assert(saoBlkParam->typeIdc >= SAO_TYPE_EO_0 && saoBlkParam->typeIdc <= SAO_TYPE_EO_45);

        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[SAO_CLASS_EO_FULL_VALLEY], SAO_CLASS_EO_FULL_VALLEY);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[SAO_CLASS_EO_HALF_VALLEY], SAO_CLASS_EO_HALF_VALLEY);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[SAO_CLASS_EO_HALF_PEAK], SAO_CLASS_EO_HALF_PEAK);
        rate += aec_write_sao_offset_fastrdo(h, p_aec, saoBlkParam->offset[SAO_CLASS_EO_FULL_PEAK], SAO_CLASS_EO_FULL_PEAK);
    }
...
/* ---------------------------------------------------------------------------
 */
int write_sao_type_fastrdo(xavs2_t *h, aec_t *p_aec, SAOBlkParam *saoBlkParam)
{
    int rate = 0;
    int val;
...
/* ---------------------------------------------------------------------------
 */
int aec_write_alf_lcu_ctrl_fastrdo(xavs2_t *h, aec_t *p_aec, uint8_t iflag)
{
    int org_bits = rdo_get_written_bits(p_aec);
...
int write_cu_header_fastrdo(xavs2_t *h, aec_t *p_aec, cu_t *p_cu)
{
    ...
    if (IS_INTRA_MODE(mode)) {
        ...
        for (i = 0; i < num_of_intra_block; i++) {
            rate += aec_write_intra_pred_mode_fastrdo(h, p_aec, p_cu->cu_info.pred_intra_modes[i]);
        }
